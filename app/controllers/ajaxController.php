...
class ajaxController extends Controller
{
  ...
  function get_materias_disponibles_profesor()
  {
    try {
      if (!check_get_data(['_t', 'id_profesor'], $_GET) || !Csrf::validate($_GET["_t"])) {
        throw new Exception(get_notification());
      }
      ...
      $materias = materiaModel::disponible_profesor($profesor['id']);
  ...
  function add_materia_profesor()
  {
    try {
      ...
      if (materiaModel::list('tb_materias_profesores', ['id_materia' => $id_materia, 'id_profesor' => $id_profesor])) {
        throw new Exception(sprintf('La materia <b>%s</b> ya esta asignada al profesor <b>%s</b>.', $materia['nombre'], $profesor['nombre']));
      }

      //asignar la materia al profesor
      if (profesorModel::asignar_materia($id_profesor, $id_materia) === false) {
        throw new Exception('asignada con exito');
      }

      $mgs = sprintf('Nueva materia <b>%s</b> asignada con exito.', $materia['nombre']);

      json_output(json_build(201, $profesor, $mgs));
    } catch (Exception $e) {
      json_output(json_build(400, null, $e->getMessage()));
  ...
  function quitar_materia_profesor()
  {
    try {
      ...
      if (!materiaModel::list('tb_materias_profesores', ['id_materia' => $id_materia, 'id_profesor' => $id_profesor])) {
        throw new Exception(sprintf('La materia <b>%s</b> no esta asignada al profesor <b>%s</b>.', $materia['nombre'], $profesor['nombre']));
      }

      //quitar la materia al profesor
      if (profesorModel::quitar_materia($id_profesor, $id_materia) === false) {
        throw new Exception('asignada con exito');
      }

      $mgs = sprintf('Nueva materia <b>%s</b> ha sido removida del profesor con exito.', $materia['nombre']);

      json_output(json_build(200, $profesor, $mgs));
    } catch (Exception $e) {
      json_output(json_build(400, null, $e->getMessage()));
    } catch (PDOException $e) {
      json_output(json_build(400, null, $e->getMessage()));
